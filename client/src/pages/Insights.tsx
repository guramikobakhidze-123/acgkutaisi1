    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="bg-white border-b py-24">
        <div className="container max-w-4xl text-center">
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-6 text-slate-900">Insights & News</h1>
          <p className="text-xl text-slate-600 leading-relaxed">
            Expert perspectives on regulatory changes, market trends, and financial strategies.
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {posts?.map((post) => (
              <Link key={post.id} href={`/insights/${post.slug}`}>
                <article className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 h-full flex flex-col cursor-pointer group">
